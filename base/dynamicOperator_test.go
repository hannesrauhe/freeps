...
func TestDynmaicOperator(t *testing.T) {
	...
	argmap := gops.GetArgSuggestions("DynFunc", "DynTestArg", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 1, "Expected 1 argument, got %v", argmap)
	argmap = gops.GetArgSuggestions("DynFunc", "DynTestArg2", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 0, "Expected 0 arguments, got %v", argmap)
	argmap = gops.GetArgSuggestions("DynFunc2", "DynTestArg", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 0, "Expected 0 arguments, got %v", argmap)
	argmap = gops.GetArgSuggestions("Simple1", "DynTestArg", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 0, "Expected 0 arguments, got %v", argmap)
	argmap = gops.GetArgSuggestions("Simple2", "DynTestArg", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 0, "Expected 0 arguments, got %v", argmap)
	argmap = gops.GetArgSuggestions("StaticFunc", "DynTestArg", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 0, "Expected 0 arguments, got %v", argmap)
	argmap = gops.GetArgSuggestions("StaticFunc", "Arg1", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 1, "Expected 1 argument, got %v", argmap)
	argmap = gops.GetArgSuggestions("StaticFunc", "CommonArg", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 3, "Expected 3 argument, got %v", argmap)
	argmap = gops.GetArgSuggestions("StaticFunc", "CommonArG2", MakeEmptyFunctionArguments())
	assert.Assert(t, len(argmap) == 1, "Expected 1 argument, got %v", argmap)
}
